...
BVHBuildNode* BVHAccel::recursiveBuild_SAH(std::vector<Object*> objects)
{
    ...
    if (objects.size() == 1) {
        ...
        {
            auto middling = objects.begin() + std::max((int)(objects.size() / B_size) * i, 1);
            ...
            float cost_AB = leftshapes.size() * SA / SC + rightshapes.size() * SB / SC + 0.125f;
            if (cost_AB < mincost)
            {
                mincost = cost_AB;
                minCostIdx = i;
        ...
        auto middling_mincost = beginning + std::max(1, (int)(objects.size() / B_size * minCostIdx));
